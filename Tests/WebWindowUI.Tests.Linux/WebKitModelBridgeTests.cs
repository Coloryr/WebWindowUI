...
public class WebKitModelBridgeTests
{
    ...
    public async Task Snapshot_ReachesDom()
    {
        ...
        await WebKitTestHarness.RunMainWindowAsync(model, async win =>
        {
            ...
            await WebKitTestHarness.WaitJsAsync(win, "window.__model.count === 7", "快照 count");
            await WebKitTestHarness.WaitJsAsync(win, "window.__model.name === \"小明\"", "快照 name");
            await WebKitTestHarness.WaitJsAsync(win, "window.__model.message === \"hi\"", "快照 message");
            await WebKitTestHarness.WaitJsAsync(win, "JSON.stringify(window.__model.extra) === '{\"a\":1}'", "快照 extra");
        }, Timeout);
    }
    ...
    public async Task IncrementalUpdate_ReachesDom()
    {
        ...
        await WebKitTestHarness.RunMainWindowAsync(model, async win =>
        {
            await WebKitTestHarness.WaitJsAsync(win, "window.__model.count === 5", "初始快照");
    ...
    public async Task FrontendWriteBack_DotNet()
    {
        ...
        await WebKitTestHarness.RunMainWindowAsync(model, async win =>
        {
            await WebKitTestHarness.WaitJsAsync(win, "window.__model.count === 7", "就绪");

            // 前端 → .NET：ModelSet → TrySetProperty → 属性写回
            await win.ExecuteScriptAsync("window.__model.name = 'from-js'; 0");
            await WebKitTestHarness.WaitDotNetAsync(() => model.Name == "from-js", ".NET Name 被前端回写");
    ...
    public async Task NulBytes_ThroughStringChannel()
    {
        ...
        await WebKitTestHarness.RunMainWindowAsync(model, async win =>
        {
            await WebKitTestHarness.WaitJsAsync(win, "window.__model.count === 1", "就绪");

            // message = "a\0b"：索引 1 是 NUL（charCode 0）——evaluate_javascript 字符串通道 + 编解码器不丢 NUL
    ...
    public async Task Extra_Object_Bidirectional()
    {
        ...
        await WebKitTestHarness.RunMainWindowAsync(model, async win =>
        {
            await WebKitTestHarness.WaitJsAsync(win, "window.__model.count === 2", "就绪");
            ...
            await WebKitTestHarness.WaitDotNetAsync(
    ...
    public async Task Settings_MultiModel_RealRun()
    {
        ...
        await WebKitTestHarness.RunWindowAsync("settings", "设置", model, async win =>
        {
            await WebKitTestHarness.WaitJsAsync(win, "window.__model.theme === 'light'", "设置快照 theme");
            // int64 字段：protobufjs Long → number 转换后才是 1048576
            await WebKitTestHarness.WaitJsAsync(win, "window.__model.totalBytes === 1048576", "int64 Long→number");
            await WebKitTestHarness.WaitJsAsync(win, "window.__model.tags.length === 2 && window.__model.tags[0] === 'bridge'", "tags 数组");
            await WebKitTestHarness.WaitJsAsync(win, "window.__model.config && window.__model.config.proxy === 'auto'", "config 对象");
            await WebKitTestHarness.WaitJsAsync(win, "window.__model.syncMode === 0", "枚举兜底 number");

            // 回写 int64：JS number → .NET long
            await win.ExecuteScriptAsync("window.__model.totalBytes = 2048; 0");
            await WebKitTestHarness.WaitDotNetAsync(() => model.TotalBytes == 2048L, "回写 totalBytes");
        }, Timeout);
    }
    ...
    public async Task SharedModel_OneInstance_TwoWindows_Broadcast()
    {
        ...
        await WebKitTestHarness.RunTwoWindowsSharedModelAsync(model, async (winA, winB) =>
        {
            // 两个窗口都收初始快照（各自订阅了同一模型实例），只读标签 instanceId 照常下发
            await WebKitTestHarness.WaitJsAsync(winA, "window.__model.count === 5 && window.__model.name === \"小明\" && window.__model.instanceId === \"共享实例\"", "A 快照");
            await WebKitTestHarness.WaitJsAsync(winB, "window.__model.count === 5 && window.__model.name === \"小明\"", "B 快照");

            // .NET 侧改动 → 广播给所有绑定窗口
            model.Count = 42;
            await WebKitTestHarness.WaitJsAsync(winA, "window.__model.count === 42", "A 收 .NET 广播");
            await WebKitTestHarness.WaitJsAsync(winB, "window.__model.count === 42", "B 收 .NET 广播");

            // 前端 A 回写 → .NET 应用后排除源窗口广播 → B 跟随，A 不回声
            await winA.ExecuteScriptAsync("window.__model.name = 'from-A'; 0");
            await WebKitTestHarness.WaitDotNetAsync(() => model.Name == "from-A", ".NET Name 被 A 回写");
            await WebKitTestHarness.WaitJsAsync(winB, "window.__model.name === \"from-A\"", "B 收到跨窗口广播");
    ...
    public async Task Launcher_RequestWriteBack_ReachesDotNet()
    {
        ...
        await WebKitTestHarness.RunWindowAsync("launcher", "示例入口", model, async win =>
        {
            ...
            await WebKitTestHarness.WaitJsAsync(win, "window.__model.request === ''", "快照 request");

            // 模拟按钮点击（等价 open('todos')）：回写窗口路径 → .NET Request
            await win.ExecuteScriptAsync("window.__model.request = 'todos'; 0");
            await WebKitTestHarness.WaitDotNetAsync(() => model.Request == "todos", ".NET Request 被前端回写");
    ...
    public async Task Launcher_CommandInvoke_ExecutesDotNetCommand()
    {
        ...
        await WebKitTestHarness.RunWindowAsync("launcher", "示例入口", model, async win =>
        {
            ...
            await WebKitTestHarness.WaitDotNetAsync(() => model.ButtonEnable, "buttonEnable 回写 .NET");

            // 带参命令现在可执行 → 参数 "todos" 传回 .NET → OpenRequested("todos")
            await win.ExecuteScriptAsync("window.__model.commandWithArg('todos'); 0");
            await WebKitTestHarness.WaitDotNetAsync(() => opened == "todos", "带参命令触发 .NET");
        }, Timeout);
    }
    ...
    public async Task DemoWindow_MultiModelHost_RoutesRegisteredInstance_WhenNotCurrent()
    {
        ...
        await WebKitTestHarness.RunWindowAsync("demo", "综合演示", demo, async win =>
        {
            // 综合页测试模式 ?model=demo：绑定 DemoModel 并捕获注入实例 id
            await WebKitTestHarness.WaitJsAsync(win, "window.__model.switchModel !== undefined", "demo 命令就绪");
            ...
            await WebKitTestHarness.WaitDotNetAsync(() => switched == "todos", "已注册非当前实例命令经路由执行");
        }, Timeout);
    }
    ...
    public async Task DemoPage_ManualMode_TabBar_CommandsRouteAndShowModelData()
    {
        ...
        await GtkPump.Instance.RunAsync(async () =>
        {
            ...
            try
            {
                ...
                await WebKitTestHarness.WaitJsAsync(win, "typeof window.__model === 'object'", "桥接就绪");
                // 目录 tab 就绪（= demoModel 已捕获实例 id 后 switchTo('launcher') 完成）
                await WebKitTestHarness.WaitJsAsync(win, "typeof window.__model.request === 'string'", "目录 tab 就绪");
                ...
                await WebKitTestHarness.WaitJsAsync(win,
                ...
                await WebKitTestHarness.WaitDotNetAsync(() => platformAction == "notify", "平台命令路由执行");
                ...
                await WebKitTestHarness.WaitDotNetAsync(() => multiOpened, "openMulti 路由到 demo 实例");
            }
            finally
            {
                win.Close();
    ...
    public async Task TodoList_TypedList_Bidirectional()
    {
        ...
        await WebKitTestHarness.RunWindowAsync("todos", "待办列表", model, async win =>
        {
            ...
            await WebKitTestHarness.WaitJsAsync(win, "window.__model.todos.length === 3 && window.__model.todos[2].title === \"t3\"", ".NET 追加 t3");

            // 前端改元素字段 → 整列表回写 .NET（桥按 proto 字段号序数键序列化元素，.NET 序数转换器重建）
            await win.ExecuteScriptAsync("window.__model.todos[0].title = 'renamed'; 0");
            await WebKitTestHarness.WaitDotNetAsync(() => model.Todos[0].Title == "renamed", ".NET todos[0] 被前端回写");
            Assert.True(model.Todos[0].Done); // 未动字段保持

            // .NET 删除：RemoveAt → Remove 差量补丁 → 前端数组原地 splice，不重建其余元素
            model.Todos.RemoveAt(0);
            await WebKitTestHarness.WaitJsAsync(win, "window.__model.todos.length === 2 && window.__model.todos[0].title === \"t2\"", ".NET 删除首个");
        }, Timeout);
    }
    ...
    public async Task Nested_SingleModelProperty_ReachesDom_And_Repush()
    {
        ...
        await WebKitTestHarness.RunWindowAsync("nested", "模型嵌套", model, async win =>
        {
            ...
            detail.Name = "改后";
            await WebKitTestHarness.WaitJsAsync(win, "window.__model.detail['1'] === \"改后\"", "父窗口重推嵌套 Detail");
        }, Timeout);
    }
    ...
    public async Task NestedDetailWindow_SameInstance_Bidirectional()
    {
        ...
        await WebKitTestHarness.RunWindowAsync("nested-detail", "嵌套详情", detail, async win =>
        {
            await WebKitTestHarness.WaitJsAsync(win, "window.__model.name === \"初始\" && window.__model.level === 1", "子窗口强类型快照");

            await win.ExecuteScriptAsync("window.__model.name = 'from-js'; 0");
            await WebKitTestHarness.WaitDotNetAsync(() => detail.Name == "from-js", "子窗口回写同一实例");

            await win.ExecuteScriptAsync("window.__model.level = 5; 0");
            await WebKitTestHarness.WaitDotNetAsync(() => detail.Level == 5, "子窗口回写 level");
        }, Timeout);
    }
    ...
    public async Task NestedListItemWindow_Tags_TypedRepeated_Bidirectional()
    {
        ...
        await WebKitTestHarness.RunWindowAsync("nested-list-item", "列表项详情", item, async win =>
        {
            await WebKitTestHarness.WaitJsAsync(
                win,
            ...
            await WebKitTestHarness.WaitDotNetAsync(() => item.Tags.Count == 2 && item.Tags[1].Name == "新增", "前端加标签回写 .NET");

            // 前端改标签名 → .NET 更新
            await win.ExecuteScriptAsync("window.__model.tags[0].name = '改名'; 0");
            await WebKitTestHarness.WaitDotNetAsync(() => item.Tags[0].Name == "改名", "前端改标签名回写 .NET");
        }, Timeout);
    }
    ...
    public async Task NestedList_Parent_Items_Snapshot_And_Repush()
    {
        ...
        await WebKitTestHarness.RunWindowAsync("nested-list", "List嵌套", model, async win =>
        {
            ...
            item.Title = "改后";
            await WebKitTestHarness.WaitJsAsync(win, "window.__model.items[0].title === \"改后\"", "父窗口重推 Items");

            // 元素内层 tags 增删（等价子窗口在根层改）→ 父模型重推
            item.Tags.Add(new NestedItemTagModel { Name = "新增" });
            await WebKitTestHarness.WaitJsAsync(win, "window.__model.items[0].tags.length === 2", "嵌套 tags 增推");
        }, Timeout);
    }
    ...
    public async Task ObservableDictionary_DotNetMutation_AutoPushesToFrontend()
    {
        ...
        await WebKitTestHarness.RunWindowAsync("nested-list", "List嵌套", model, async win =>
        {
            ...
            await WebKitTestHarness.WaitJsAsync(win, "window.__model.counts.items === 99", ".NET 原地改 dict[k] 自动推送");

            // .NET 侧新增键 → 自动推前端
            model.Counts["extra"] = 5;
            await WebKitTestHarness.WaitJsAsync(win, "window.__model.counts.extra === 5", ".NET Add 新键自动推送");
        }, Timeout);
    }
    ...
    public async Task ObservableDictionary_FrontendEdit_WritesBackToDotNet()
    {
        ...
        await WebKitTestHarness.RunWindowAsync("nested-list", "List嵌套", model, async win =>
        {
            await WebKitTestHarness.WaitJsAsync(win, "window.__model.counts && window.__model.counts.items === 3", "就绪 counts");

            // 前端原地改字典值 → 深 watch 整字典 name 键回写 .NET（整属性重建）
            await win.ExecuteScriptAsync("window.__model.counts.items = 7; 0");
            await WebKitTestHarness.WaitDotNetAsync(() => model.Counts["items"] == 7, ".NET Counts[items] 被前端回写");
            Assert.Equal(4, model.Counts["tags"]); // 未动键保持

            // 前端新增键 → .NET 出现
            await win.ExecuteScriptAsync("window.__model.counts.extra = 2; 0");
            await WebKitTestHarness.WaitDotNetAsync(() => model.Counts.TryGetValue("extra", out int v) && v == 2, ".NET Counts[extra] 前端新增回写");
        }, Timeout);
    }
    ...
    public async Task GetOnlyCollection_FrontendPush_ReachesDotNet()
    {
        ...
        await WebKitTestHarness.RunWindowAsync("nested-list", "List嵌套", model, async win =>
        {
            await WebKitTestHarness.WaitJsAsync(win, "window.__model.items.length === 1", "快照 items");
