...
public static class ModelProtocol
{
    ...
    public static bool TryFromModelValue(ModelValue? value, Type targetType, out object? result)
    {
        ...
        bool empty = value.Number is null && value.Text is null && value.Flag is null
    ...
    private static object? ConvertNumber(double d, Type targetType)
    {
        ...
        if (t.IsEnum)
        {
            Type under = Enum.GetUnderlyingType(t);
    ...
    private static bool TryConvertList(ModelValueList list, Type targetType, out object? result)
    {
        ...
        if (t == typeof(object))
        {
            ...
            foreach (ModelValue item in list.Items)
            {
        ...
        Type? elemType = t.IsArray ? t.GetElementType()
        ...
        Type listType = t.GetGenericTypeDefinition() == typeof(ObservableCollection<>)
    ...
    {
        foreach (KeyValuePair<string, ModelValue> kv in map.Fields)
            yield return kv;
        foreach (KeyValuePair<int, ModelValue> kv in map.OrdinalFields)
    ...
    private static bool TryConvertObject(ModelValueMap map, Type targetType, out object? result)
    {
        ...
        if (t == typeof(object) || t == typeof(Dictionary<string, object>))
        {
            ...
            foreach (KeyValuePair<string, ModelValue> kv in EnumerateMapEntries(map))
            {
            ...
            if (props.Length > 0)
            {
                ...
                if (instance is not null)
                {
                    foreach (KeyValuePair<string, ModelValue> kv in map.Fields)
                    {
                        ...
                        PropertyInfo? prop = props.FirstOrDefault(p => string.Equals(p.Name, kv.Key, StringComparison.OrdinalIgnoreCase));
                        if (prop is null)
                            continue;
