...
internal static class CefSchemes
{
    ...
    {
        byte[] payload = ReadPostDataPayload(request);
        if (TryGetWindow(browser, out CefWindow? window))
    ...
    private static CefResourceHandler CreateResourceHandler(CefWindow window, CefRequest request)
    {
        try
        {
            ...
            {
                using Stream? stream = resolver(relative!);
    ...
    internal sealed class WwuiResourceHandler : CefResourceHandler
    {
        ...
        public WwuiResourceHandler(byte[] data, string mime, int status, string? cacheControl)
        {
            _data = data;
            _mime = mime;
            _status = status;
        ...
        {
            responseLength = _data.LongLength;
            redirectUrl = string.Empty; // 不重定向（CefGlue 签名非空 out string）
            response.Status = _status;
            response.StatusText = StatusText(_status);
            ...
            if (_cacheControl is not null)
                response.SetHeaderByName("Cache-Control", _cacheControl, false);
        }
        ...
        protected override bool Skip(long bytesToSkip, out long bytesSkipped, CefResourceSkipCallback callback)
        {
            ...
            long actual = Math.Min(bytesToSkip, remaining);
            _offset += (int)actual;
            bytesSkipped = actual;
    ...
    private static byte[] ReadPostDataPayload(CefRequest request)
    {
        ...
        foreach (CefPostDataElement element in postData.GetElements())
            merged.AddRange(element.GetBytes());
        if (merged.Count == 0)
            return Array.Empty<byte>();
        // fetch 把 JS 字符串按 UTF-8 编码成 body 字节，先解码回 NUL 转义串，再还原成 protobuf 字节
        string escaped = Encoding.UTF8.GetString([.. merged]);
