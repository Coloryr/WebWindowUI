...
public abstract partial class WebWindowModel : ObservableObject
{
    ...
    {
        foreach (KeyValuePair<string, INotifyCollectionChanged> kv in _collectionSubs)
    ...
    private void PushEnvelope(byte[] bytes)
    {
        ...
        foreach (Action<byte[]> handler in snapshot)
            handler(bytes);
    }
    ...
    {
        byte[] payload = EncodePropertyUpdate(propertyName, value);
