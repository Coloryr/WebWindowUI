...
public abstract partial class BaseCefBrowser : IDisposable
{
    ...
    /// <summary>
    /// Event fired when the underlying browser is destroyed (all browsers, including popups).
    /// The argument is the destroyed browser; hosts distinguish the main browser from popups
    /// (devtools, ...) by comparing with <see cref="UnderlyingBrowser"/>.
    /// </summary>
    ...
    /// <summary>
    /// Closes the underlying browser window (optionally forced). The BrowserClosed event
    /// fires once the browser is destroyed.
    /// </summary>
    /// <param name="forceClose">Force close even if the page has unload handlers.</param>
    public void CloseBrowser(bool forceClose)
    {
        _adapter.CloseBrowser(forceClose);
    }
