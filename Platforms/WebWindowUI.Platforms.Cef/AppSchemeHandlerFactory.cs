...
/// <summary>
/// app:// / appdata:// scheme 处理器工厂：GET 服务 wwwroot 资源，POST __wwui 投递 JS 回传消息。
/// </summary>
internal sealed class AppSchemeHandlerFactory : ISchemeHandlerFactory
{
    /// <summary>
    /// 为请求创建资源处理器。
    /// </summary>
    /// <param name="browserControl">浏览器（借用）。</param>
    /// <param name="frame">帧（借用）。</param>
    /// <param name="schemeName">scheme 名。</param>
    /// <param name="request">请求（借用）。</param>
    /// <returns>资源处理器。</returns>
    public IResourceHandler Create(IBrowser browserControl, IFrame frame, string schemeName, IRequest request)
        => new AppResourceHandler(browserControl?.Identifier ?? 0, request);
}

/// <summary>
/// app:// 资源处理器：GET 读 wwwroot 资源；POST（__wwui）解码 JS 回传字节并投递对应窗口。
/// </summary>
internal sealed class AppResourceHandler : IResourceHandler
{
    /// <summary>
    ...
    public AppResourceHandler(long browserId, IRequest request)
    {
        ...
        if (WebWindowResource.TryResolvePath(request.Url, out var relative, out var mimeType) is { } stream)
        {
            ...
            // CEF 不识别带 charset 的 MIME（按纯文本显示源码），`;` 剥离。勿设 IResponse.Charset。
    ...
    /// </summary>
    /// <param name="request">请求。</param>
    private void HandlePost(IRequest request)
    {
        ...
        try
        {
            if (request.PostData is { } postData)
            {
                ...
                {
                    if (element.Type == PostDataElementType.Bytes && element.Bytes is { } bytes)
    ...
    public bool Read(Stream dataOut, out int bytesRead, Callback.IResourceReadCallback callback)
    {
        ...
        bytesRead = Math.Min(8192, _data.Length - _offset);
        dataOut.Write(_data, _offset, bytesRead);
        _offset += bytesRead;
    ...
    /// <param name="callback">异步回调（同步完成无需使用）。</param>
    /// <returns>是否还有数据。</returns>
    public bool Skip(long bytesToSkip, out long bytesSkipped, Callback.IResourceSkipCallback callback)
    {
