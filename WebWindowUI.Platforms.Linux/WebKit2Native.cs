...
internal static class WebKit2Native
{
    ...
    {
        IntPtr view = webkit_web_view_new();
        g_object_ref(view);
        lock (_liveViews)
            _liveViews.Add(view);
        return view;
    ...
    {
        int ok = webkit_user_content_manager_register_script_message_handler(GetUserContentManager(view), name);
    ...
    {
        IntPtr p = webkit_uri_scheme_request_get_uri(request);
    ...
    {
        IntPtr bytes = g_bytes_new(data, (nuint)data.Length);
    ...
    {
        IntPtr jscValue = webkit_javascript_result_get_js_value(jsResult);
    ...
    private static string JscValueToJson(IntPtr jscValue)
    {
        if (jscValue == IntPtr.Zero)
            return "null";
        IntPtr p = jsc_value_to_json(jscValue, 0);
        if (p != IntPtr.Zero)
        {
    ...
    private static string JscValueToString(IntPtr jscValue)
    {
        if (jscValue == IntPtr.Zero)
            return "";
        IntPtr p = jsc_value_to_string(jscValue);
    ...
    private static void OnEvaluateJavascriptReady(IntPtr sourceObject, IntPtr result, IntPtr userData)
    {
        ...
        IntPtr jscValue = webkit_web_view_evaluate_javascript_finish(sourceObject, result, out error);
        if (error != IntPtr.Zero)
        {
            string message = ReadAndFreeGError(error);
    ...
    private static string ReadAndFreeGError(IntPtr error)
    {
        ...
        IntPtr msg = Marshal.ReadIntPtr(error, 8);
        string text = msg == IntPtr.Zero ? "WebKit 错误" : Marshal.PtrToStringUTF8(msg) ?? "WebKit 错误";
        g_error_free(error);
        return text;
    }
}
